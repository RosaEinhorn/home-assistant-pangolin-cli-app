...
EXTRA_FLAGS=$(jq -r '.EXTRA_FLAGS // ""' "$CONFIG_PATH")
...
while true; do
    ...
    # Build command with extra flags if provided
    CMD="/usr/bin/pangolin-cli up --attach --id \"$CLIENT_ID\" --secret \"$CLIENT_SECRET\" --endpoint \"$PANGOLIN_ENDPOINT\""
    if [[ -n "$EXTRA_FLAGS" && "$EXTRA_FLAGS" != "null" ]]; then
        CMD="$CMD $EXTRA_FLAGS"
    fi
